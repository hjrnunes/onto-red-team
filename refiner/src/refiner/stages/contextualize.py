...
def _contextualize_single_risk(
    rva: RiskVariationAxes,
    client: instructor.Instructor,
    config: LLMConfig,
    onto_handlers: dict,
    selected_domains: list[str] | None,
    risk_details: dict[str, dict] | None,
    report: RunReport | None,
    policies: list[Policy] | None,
    vocabulary_contexts: dict[str, dict] | None,
    enumerations_per_axis: int,
) -> list[DomainContextAxis]:
    if not rva.axes:
        return []
...
def contextualize(
    ...
    # Collect unique risk_ids and the first RVA for each (used for processing)
    unique_rvas: dict[str, RiskVariationAxes] = {}
    for rva in variation_axes:
        if rva.risk_id not in unique_rvas and rva.axes:
            unique_rvas[rva.risk_id] = rva

    shared_kwargs = dict(
        client=client, config=config, onto_handlers=onto_handlers,
        selected_domains=selected_domains, risk_details=risk_details,
        report=report, policies=policies,
        vocabulary_contexts=vocabulary_contexts,
        enumerations_per_axis=enumerations_per_axis,
    )

    context_cache: dict[str, list[DomainContextAxis]] = {}

    max_workers = min(config.max_concurrent, len(unique_rvas)) if unique_rvas else 1

    if max_workers > 1:
        logger.info("contextualize: processing %d unique risks with %d parallel workers", len(unique_rvas), max_workers)

        def _do_risk(item: tuple[str, RiskVariationAxes]) -> tuple[str, list[DomainContextAxis]]:
            rid, rva = item
            return rid, _contextualize_single_risk(rva, **shared_kwargs)

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for rid, axes in executor.map(_do_risk, unique_rvas.items()):
                context_cache[rid] = axes
    else:
        for rid, rva in unique_rvas.items():
            context_cache[rid] = _contextualize_single_risk(rva, **shared_kwargs)

    # Risks with no axes get empty cache entry
    for rva in variation_axes:
        if rva.risk_id not in context_cache:
            context_cache[rva.risk_id] = []

    # Build results preserving original ordering
    policy_groundings: dict[str, list[RiskGrounding]] = {}
    seen_risk_ids: set[str] = set()
    risk_names: dict[str, str] = {}

    for rva in variation_axes:
        populated_axes = context_cache[rva.risk_id]
        ...
        grounding = RiskGrounding(risk_id=rva.risk_id, axes=populated_axes, axis_groups=filtered_groups)
